...
def enrich(raw, source):
    ...
    return {
        ...
        "source_url": raw.get("source_url") or (f"https://ted.europa.eu/en/notice/{publication}/html" if publication else ""),
...
def tenders():
    ...
    results = [x for x in results if x["score"] >= minimum and
               (category == "All" or category in x["matched_categories"]) and
               (country == "All" or x["country"] == country) and
               (source == "All" or x["source"] == source) and is_live(x)]
    results.sort(key=lambda x: x["score"], reverse=True)
    countries = sorted({x["country"] for x in results if x["country"]})
    sources = sorted({x["source"] for x in results if x["source"]})
    return jsonify({"updated_at": datetime.now(timezone.utc).isoformat(), "live_only": True,
                    "source_count": len(sources), "results": results[:250], "count": len(results),
                    "countries": countries, "sources": sources,
                    "categories": list(CONFIG["categories"].keys()), "source_registry": SOURCES,
                    "errors": errors})
