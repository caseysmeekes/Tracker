...
def parse_datetime(value):
    ...
    match = re.search(r"(\d{1,2}:\d{2}\s*(?:AM|PM))\s+(\d{1,2}\s+[A-Za-z]{3}\s+\d{4}).*?UTC([+-]\d{1,2})(?::(\d{2}))?", raw, re.I)
    if match:
        try:
            local = datetime.strptime(f"{match.group(2)} {match.group(1)}", "%d %b %Y %I:%M %p")
            sign = 1 if match.group(3).startswith("+") else -1
            offset = timezone(sign * timedelta(hours=abs(int(match.group(3))), minutes=int(match.group(4) or 0)))
...
def enrich(raw, source):
    ...
    return {"id":raw.get("id") or publication or title,"title":title.strip(),"description":description.strip(),
            "country":text_value(raw.get("buyer-country") or raw.get("country")),"buyer":text_value(raw.get("buyer-name") or raw.get("buyer")),
            "deadline":text_value(raw.get("deadline-receipt-tender-date-lot") or raw.get("deadline")),"published":text_value(raw.get("publication-date") or raw.get("published")),
            "notice_type":text_value(raw.get("notice-type")),"cpv":text_value(raw.get("classification-cpv")),"score":score,
            "matched_categories":categories,"source_url":raw.get("source_url") or (f"https://ted.europa.eu/en/notice/{publication}/html" if publication else ""),
...
def tenders():
    ...
    results=[x for x in results if x["score"]>=minimum and (category=="All" or category in x["matched_categories"]) and (country=="All" or x["country"]==country) and (source=="All" or x["source"]==source) and is_live(x)]
